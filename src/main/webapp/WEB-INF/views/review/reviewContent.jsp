	...
	<div class="container mt-3">
		...
		<div class="event_sub">
			...
			<img style="width: 50px"
				src="${contextPath}/resources/image/event/line.png" />
		</div>
...
<%-- 			<form action="<c:url value='/review/update'/>" method="POST"
				...
				<table>
					<tr>
						<th>주문상품</th>
						<td><input class="content_input" name="orderList" type="text"
							disabled /></td>
					</tr>
					...
					<tr>
						...
						<td><c:choose>
								<c:when test="${empty review.re_imageFileList}">
									<input class="content_input" name="orderList" type="text"
										disabled />
								</c:when>
								<c:otherwise>
									<div class="content_input_file">
										<c:forEach items="${review.re_imageFileList}"
											var="re_imageFileList">
											<input class="review_imgName" name="review_image" type="text"
												value="${re_imageFileList.re_originalFileName}" disabled />
											<br />
											<img class="review_preview"
												src="${contextPath}/review/imgDown?re_storedFileName=${re_imageFileList.re_storedFileName}"
												style="width: 200px;" />
											<br />
										</c:forEach>
									</div>
