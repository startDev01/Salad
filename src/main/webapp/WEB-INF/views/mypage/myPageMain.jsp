...
<link
	href="https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@100;300;400;500;700;900&family=Open+Sans:wght@400;500;600;700&display=swap"
	rel="stylesheet">
...
<body>
	...
	<%-- <table class="myinfo_table">
		...
		<tr>
			<td class="myinfo_td_title">주소지</td>
			<td class="myinfo_td">기본 주소: &nbsp;&nbsp; <strong>${user.userAddress1 }</strong>
	...
	<div class="info-block to">
		...
		<div class="line">
			<div class="line-title">
				<label for="phoneNb">연락처</label>
			</div>
			<div class="line-content">
				<input type="text" name="userphone" id="userphoneNb" maxlength="11"
	...
	<form method="post" id="orderhis_Form">
		<table>
			<tbody>
				...
				<tr>
					<td><a href="javascript:search_order_history('today')"> <img
							src="${contextPath}/resources/image/search/btn_search_one_day.jpg">
					</a> <a href="javascript:search_order_history('three_day')"> <img
							src="${contextPath}/resources/image/search/btn_search_3_day.jpg">
					</a> <a href="javascript:search_order_history('one_week')"> <img
							src="${contextPath}/resources/image/search/btn_search_1_week.jpg">
					</a> <a href="javascript:search_order_history('one_month')"> <img
							src="${contextPath}/resources/image/search/btn_search_1_month.jpg">
					</a> <a href="javascript:search_order_history('three_month')"> <img
							src="${contextPath}/resources/image/search/btn_search_3_month.jpg">
					</a> <a href="javascript:search_order_history('six_month')"> <img
							src="${contextPath}/resources/image/search/btn_search_6_month.jpg">
					</a> <!-- 	&nbsp;까지 조회 --></td>
				</tr>
				<br>
		...
		<tbody align="center">
			...
			<c:choose>
				<c:when test="${ empty myOrderHistList  }">
					<tr>
						<td colspan=5 class="fixed" align="center"><strong>주문한
								상품이 없습니다.</strong></td>
					</tr>
				</c:when>
				<%-- 주문 테이블 목록 출력 --%>
				<c:otherwise>
					<c:forEach var="item" items="${myOrderHistList }" varStatus="i">
						<tr>
							<td class="my_page_td"><a
								href='${contextPath}/mypage/orderInfo/${item.orderNum}'">${item.fakeOrderNum }
							</a></td>
							<td class="my_page_td">${item.orderCreateTimestamp}</td>
							<td class="my_page_td">${item.ordererName}</td>
							<td class="my_page_td"><fmt:formatNumber
									value="${item.totalPrice}" pattern="#,###" /> 원</td>
							<td class="my_page_td">${item.orderStatus}</td>
							<td class="my_page_td"><c:if
									test="${item.orderStatus == '결제완료'}">
									<input type="button" class="my_page_cancel"
										onclick="fn_cancel_order('${item.orderNum}')" value="주문취소" />
								</c:if></td>
						</tr>
					</c:forEach>
				</c:otherwise>
